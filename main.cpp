...
enum GameState { MENU, PLAY, GAMEOVER, SETTINGS, SOUND_MENU, HELP_MENU, pMENU,HOME, LEVEL_MENU};
...
void draw_gameover_options(){
   ...
   // iClear();
    ...
    if(score>=1500){
    iTextBold(277, 520, "YOU WIN!", GLUT_BITMAP_TIMES_ROMAN_24);
    }
    else{
...
void iKeyboard(unsigned char key){
    if(key=='b') current_state=MENU;
    if(key=='a' && ship.x>0){
        ship.x-=10;
    }
    if(key=='d' && ship.x+80<700){
        ship.x+=10;
    }
    if(key=='r') reset();
